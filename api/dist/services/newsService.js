...
class NewsService {
    constructor() {
        this.apiKey = process.env.NEWSAPI_API_KEY || config_1.config.newsapi.apiKey;
        if (!this.apiKey) {
    ...
    async testApiConnection() {
        try {
            ...
            const sourcesResponse = await axios_1.default.get(sourcesUrl, { params: sourcesParams });
            ...
            const params = {
                apiKey: this.apiKey,
                sources: sourcesResponse.data.sources.map((source) => source.id).join(','),
                pageSize: 10,
                page: 1
            };
            console.log("[NewsService] NewsAPI.org URL:", testUrl);
            console.log("[NewsService] Test params:", params);
            const response = await axios_1.default.get(testUrl, { params });
        ...
        for (const article of articles) {
            try {
                ...
                try {
                    ...
                    const savedArticle = await NewsArticle_1.NewsArticle.create({
                        title: article.title,
     ...
     */
    async getTopHeadlines(category, lang = 'en', limit = 20) {
        try {
            // Check if we have cached articles in MongoDB
            const cachedArticles = await NewsArticle_1.NewsArticle.find()
                .sort({ publishedAt: -1 })
                .limit(limit)
                .lean();
            const now = new Date();
            ...
            try {
                ...
                const response = await axios_1.default.get(`${this.baseUrl}/top-headlines`, { params });
                ...
                const freshArticles = await NewsArticle_1.NewsArticle.find()
                    .sort({ publishedAt: -1 })
                    .limit(limit)
                    .lean();
     ...
     */
    async searchNews(query, lang = 'en', max = 20) {
        try {
            ...
            try {
                const response = await axios_1.default.get(`${this.baseUrl}/top-headlines`, {
                    params: {
                        apiKey: this.apiKey,
                        language: lang || 'en',
                        country: 'in',
