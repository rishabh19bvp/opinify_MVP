...
describe('Poll Model', () => {
    ...
    describe('Creation', () => {
        ...
        it('should throw error for invalid title', async () => {
            ...
            await expect(Poll_1.Poll.create({
                ...invalidPoll,
                creator: testUser
        ...
        it('should allow empty options array', async () => {
            ...
            const result = await Poll_1.Poll.create({
                ...invalidPoll,
                creator: testUser
            });
        ...
        it('should throw error for invalid status', async () => {
            const invalidPoll = { ...testPollData, status: 'invalid' };
            await expect(Poll_1.Poll.create({
                ...invalidPoll,
                creator: testUser
    ...
    describe('Poll Methods', () => {
        let testPoll;
        beforeEach(async () => {
            testPoll = await Poll_1.Poll.create({
                ...testPollData,
                creator: testUser
            });
        });
        it('should check if poll is expired', async () => {
            const now = new Date();
            const expiredPoll = await Poll_1.Poll.create({
                ...testPollData,
                creator: testUser,
    ...
    describe('Poll Queries', () => {
        it('should find polls by creator', async () => {
            ...
            const foundPolls = await Poll_1.Poll.findByCreator(testUser);
        ...
        it('should find polls by category', async () => {
            ...
            const foundPolls = await Poll_1.Poll.findByCategory(testPollData.category);
            expect(foundPolls.length).toBe(1);
            expect(foundPolls[0].id).toBe(poll.id);
        });
    });
